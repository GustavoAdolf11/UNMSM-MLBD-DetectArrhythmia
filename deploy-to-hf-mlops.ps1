# ============================================================================
# Script de Despliegue a Hugging Face con Integración MLOps
# ============================================================================
# Este script:
# 1. Busca el mejor modelo en MLflow (por F1-Score de clase V)
# 2. Lo copia al directorio de producción
...
Write-Host "║   🚀 Despliegue a Hugging Face con MLOps                   ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
# ============================================================================
Write-Host "📊 Buscando mejor modelo en MLflow..." -ForegroundColor Cyan
Write-Host ""
...
try:
    ...
    f1_v = best_run.get('metrics.test_f1_V', 0.0)
    accuracy = best_run.get('metrics.test_accuracy', 0.0)
    precision_v = best_run.get('metrics.test_precision_V', 0.0)
    recall_v = best_run.get('metrics.test_recall_V', 0.0)
    ...
    # Guardar métricas en archivo JSON para el commit
    metrics_info = {
...
# Copiar modelo al directorio de producción
try:
    ...
    if model_src.exists():
        ...
        print('HINT:Verifica que el entrenamiento guardó el modelo correctamente')
        exit(1)
...
if (-not $success) {
    Write-Host ""
    Write-Host "❌ Error al procesar MLflow. Abortando despliegue." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "❌ Error al preparar archivos: $_" -ForegroundColor Red
    exit 1
}

# ============================================================================
# PASO 3: Verificar que el remoto de HF está configurado
# ============================================================================
...
if (-not $hfRemote) {
    Write-Host "⚠️  Remoto 'hf' no configurado" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "O ejecuta el script deploy-to-hf.ps1 primero para configuración inicial" -ForegroundColor Yellow
    exit 1
}

Write-Host "✅ Remoto HF configurado: $hfRemote" -ForegroundColor Green
Write-Host ""
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
        Write-Host "📊 Modelo desplegado:" -ForegroundColor Cyan
        Write-Host "   F1-Score V: $f1Score" -ForegroundColor White
        Write-Host "   Accuracy: $accuracy" -ForegroundColor White
        Write-Host ""
        Write-Host "🔗 Accede a tu Space:" -ForegroundColor Cyan
        Write-Host "   $hfRemote" -ForegroundColor White
        Write-Host ""
        Write-Host "⏳ El build puede tardar 5-10 minutos" -ForegroundColor Yellow
        Write-Host "   Monitorea el progreso en Hugging Face" -ForegroundColor Yellow
        Write-Host ""
    }
    else {
        Write-Host ""
        Write-Host "❌ Error al hacer push a Hugging Face" -ForegroundColor Red
